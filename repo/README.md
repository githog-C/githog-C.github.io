...
**下載 PDF（列印）**：按鈕會打開瀏覽器的列印對話框，選「另存為 PDF」即可。簡報的列印樣式把頁面固定為 1920×1080、邊界 0，所以不管視窗多大，每一頁都是無白邊的 16:9 滿版；背景色與緞帶已強制列印，不必另外勾「背景圖形」（勾了也無妨）。要印成 A4 時，用印表機的「縮放至頁面」即可，短邊會留白，內容不裁切。下載出的簡報檔右上角也有「PDF」鈕，Ctrl+P 效果相同。
