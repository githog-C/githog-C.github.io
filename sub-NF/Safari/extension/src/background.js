...
api.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Accept only Netflix caption CDNs.
  let host = '';
  try { host = new URL(msg.url).hostname; } catch (_) { sendResponse({ ok: false, error: 'bad url' }); return; }
  const allowed = /(^|\.)nflxvideo\.net$/.test(host) || /(^|\.)nflxext\.com$/.test(host);
  if (!allowed) { sendResponse({ ok: false, error: 'host not allowed' }); return; }
